        ...
        const unsigned int n_train_samples, const float *train_X, const float *train_Y )
{
    ...
    static float beta_1_corrected = adam->beta_1;
    static float beta_2_corrected = adam->beta_2;
    ...
    for ( unsigned int i = 0; i < n_train_samples ;  ){
        ...
        update_biased_first_moment( n_parameters, opt->s, g, opt->beta_1 );
        update_biased_second_moment( n_parameters, opt->r, g, opt->beta_2 );

        accumulate_squared_gradient( n_parameters, r, delta_w, adam->rho );
        compute_update( n_parameters, delta_w, r, adam->learning_rate /*, epsilon? */ );
